 ...
 * \par KA_thod List:
 *
 *    1. void KA_RGBLed::reset(uint8_t port)
 *    2. void KA_RGBLed::reset(uint8_t port,uint8_t slot)
 *    3. void KA_RGBLed::setpin(uint8_t port)
 *    4. uint8_t KA_RGBLed::getNumber()
 *    5. cRGB KA_RGBLed::getColorAt(uint8_t index)
 *    6. bool KA_RGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
 *    7. bool KA_RGBLed::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
 *    8. bool KA_RGBLed::setColor(uint8_t red, uint8_t green, uint8_t blue)
 *    9. bool KA_RGBLed::setColor(uint8_t index, long value)
 *    10. void KA_RGBLed::show()
 *
 * \par History:
 * <pre>
...
/**
 * Alternate Constructor which can call your own function to map the KA_RGBLed to arduino port,
 * no pins are used or initialized here, it only assigned the LED display buffer. The default
 *number of light strips is 32.
 * \param[in]
 *   None
 */
KA_RGBLed::KA_RGBLed(void) : KA_Port()
{
 ...
 */
KA_RGBLed::KA_RGBLed(uint8_t port) : KA_Port(port)
{
 ...
 */
KA_RGBLed::KA_RGBLed(uint8_t port, uint16_t led_num) : KA_Port(port)
{
  pinMask       = digitalPinToBitMask(s1);
  ws2812_port   = portOutputRegister(digitalPinToPort(s1) );
  //set pinMode OUTPUT */
 ...
 *   led_num - The LED number
 */
KA_RGBLed::KA_RGBLed(uint8_t port, uint8_t slot, uint16_t led_num) : KA_Port(port)
{
  if(slot == SLOT1)
  {
 ...
 *   None
 */
void KA_RGBLed::reset(uint8_t port)
{
  KA_Port::reset(port);
  pinMask = digitalPinToBitMask(s1);
  ws2812_port = portOutputRegister(digitalPinToPort(s1) );
 ...
 *   None
 */
void KA_RGBLed::reset(uint8_t port,uint8_t slot)
{
  KA_Port::reset(port);
  if(SLOT2 == slot)
  {
 ...
 *   None
 */
void KA_RGBLed::setpin(uint8_t port)
{
 ...
 *   None
 */
void KA_RGBLed::setNumber(uint16_t num_leds)
{
  count_led = num_leds;
  ...
  for(uint16_t i = 0; i <= count_led * 3; i++)
  {
    pixels[i] = 0;
  }
}
 ...
 */
cRGB KA_RGBLed::getColorAt(uint16_t index)
{
  cRGB px_value;
 ...
 *   The index value from 1 to the max
 */
uint16_t KA_RGBLed::getNumber(void)
{
  return(count_led);
}
 ...
 */
bool KA_RGBLed::setColorAt(uint16_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index <= count_led)
  {
 ...
 */
bool KA_RGBLed::setColor(uint16_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index == 0)
  {
 ...
 */
bool KA_RGBLed::setColor(uint8_t red, uint8_t green, uint8_t blue)
{
...
bool KA_RGBLed::setColor(uint16_t index, uint16_t value)
{
  if(index == 0)
  {
    for(uint16_t i = 0; i <= count_led; i++)
    {
      uint16_t tmp    = i * 3;
      uint8_t red    = (value >> 11) & 0x1f;
      uint8_t green  = (value >> 5) & 0x1f;
      uint8_t blue   = value & 0x1f;
      pixels[tmp]    = green;
      pixels[tmp + 1] = red;
      pixels[tmp + 2] = blue;
    }
 ...
 *   None
 */
void KA_RGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
