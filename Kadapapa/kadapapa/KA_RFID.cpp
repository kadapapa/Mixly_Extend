/*
 * KA_RFID.cpp - Library to use ARDUINO KA_RFID MODULE KIT 13.56 MHZ WITH TAGS SPI W AND R BY COOQROBOT.
...
/******************************************************************************
 * 构造 KA_RFID
 * int chipSelectPin KA_RFID /ENABLE pin
 ******************************************************************************/
KA_RFID::KA_RFID(char port):KA_Port(port)
{
  _chipSelectPin =pin2();
 ...
 * 返 回 值：无
 ******************************************************************************/
void KA_RFID::writeMFRC522(unsigned char addr, unsigned char val)
{
  digitalWrite(_chipSelectPin, LOW);
 ...
 * 返 回 值：返回读取到的一个字节数据
 ******************************************************************************/
unsigned char KA_RFID::readMFRC522(unsigned char addr)
{
  unsigned char val;
  digitalWrite(_chipSelectPin, LOW);
 ...
 * 返 回 值：无
 ******************************************************************************/
void KA_RFID::setBitMask(unsigned char reg, unsigned char mask)
{
  unsigned char tmp;
  tmp = readMFRC522(reg);
 ...
 * 返 回 值：无
 ******************************************************************************/
void KA_RFID::clearBitMask(unsigned char reg, unsigned char mask)
{
  unsigned char tmp;
  tmp = readMFRC522(reg);
 ...
 * 返 回 值：无
 ******************************************************************************/
void KA_RFID::calculateCRC(unsigned char *pIndata, unsigned char len, unsigned char *pOutData)
{
  unsigned char i, n;
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
unsigned char KA_RFID::MFRC522ToCard(unsigned char command, unsigned char *sendData, unsigned char sendLen, unsigned char *backData, unsigned int *backLen)
{
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
unsigned char KA_RFID::MFRC522Request(unsigned char reqMode, unsigned char *TagType)
{
  unsigned char status;
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
unsigned char KA_RFID::anticoll(unsigned char *serNum)
{
  unsigned char status;
  unsigned char i;
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
unsigned char KA_RFID::auth(unsigned char authMode, unsigned char BlockAddr, unsigned char *Sectorkey, unsigned char *serNum)
{
  unsigned char status;
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
unsigned char KA_RFID::read(unsigned char blockAddr, unsigned char *recvData)
{
  unsigned char status;
  unsigned int unLen;
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
unsigned char KA_RFID::write(unsigned char blockAddr, unsigned char *writeData)
{
  unsigned char status;
  unsigned int recvBits;
  unsigned char i;
...
unsigned char KA_RFID::selectTag(unsigned char *serNum)
{
  unsigned char i;
  unsigned char status;
  unsigned char size;
...
unsigned char KA_RFID::ReadAction()
{	
	RecStrData[8] = 0x00;
	...
	if((auth(PICC_AUTHENT1A, 7, KA_SectorKey, serNum)) == MI_OK)
	{			
		if(read(4, RecStrData) == MI_OK)
		{
			buzzer.tone(1047,125);
		}
 ...
 * 返 回 值：成功返回MI_OK
 ******************************************************************************/
bool KA_RFID::WriteAction(unsigned char Data)
{
	isCard();//寻卡
	...
	if((auth(PICC_AUTHENT1A, 7, Init_SectorKey, serNum)) == MI_OK)//验证密钥
	{				
		write(7, KA_SectorKey);//密钥重置为KADAPAPA私有密钥				
	}
	...
	if((auth(PICC_AUTHENT1A, 7, KA_SectorKey, serNum)) == MI_OK)//验证密钥
	{	
		WriStrData[8] = Data;	
		
		if(write(4, WriStrData) == MI_OK)
		{
			buzzer.tone(1047,125);
			return true;
		}			
