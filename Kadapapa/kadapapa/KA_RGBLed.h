 ...
 * @version V1.0.0
 * @date    2015/09/01
 * @brief   Header for KA_RGBLed.cpp module
 *
 * \par Copyright
 ...
 *
 */
#ifndef __KA_RGBLED_h
#define __KA_RGBLED_h
#include <stdint.h>
#include <stdbool.h>
#include <Arduino.h>
#include "KA_Port.h"

#define DEFAULT_MAX_LED_NUMBER  (256)
 ...
 *
 * \par Description
 * Declaration of Class KA_RGBLed
 */

class KA_RGBLed : public KA_Port
{
public:
 ...
 */
  KA_RGBLed(uint8_t port, uint16_t led_num);
 ...
 */
  ...
  KA_RGBLed(uint8_t port, uint8_t slot, uint16_t led_num);
 ...
 */
  bool setColorAt(uint16_t index, uint8_t red, uint8_t green, uint8_t blue);
 ...
 */
  bool setColor(uint16_t index, uint8_t red, uint8_t green, uint8_t blue);
 ...
 *   The index value from 1 to the max, if you set the index 0, all the LED will be lit
 */
  bool setColor(uint16_t index, uint16_t value);
 ...
 *   None
 */
  void setNumber(uint16_t num_led);
