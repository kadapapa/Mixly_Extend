/* KA_RFID.h - Library to use ARDUINO KA_RFID MODULE KIT 13.56 MHZ WITH TAGS SPI W AND R BY COOQROBOT.
...
class KA_RFID:public KA_Port
{
  public:
	...
	void halt();
	unsigned char ReadAction();
	bool WriteAction(unsigned char Data);
	
	unsigned char serNum[5];       // 4字节卡序列号，第5字节为校验字节

	unsigned char RecStrData[MAX_LEN]; //读数据临时存储区
	unsigned char WriStrData[MAX_LEN] =
	{
		/**************************************************
		0xff  0x55  len   idx action device port  slot  data  
		0     1     2     3     4     5     6     7     8
		***************************************************/
		0xFF, 0x55, 0x0D, 0x00, 0x01, 0x6E, 0x02, 0x00, 0x00, 0X00, 0x00, 0X00, 0x00, 0X00, 0x00, 0X0A
	}; //写数据临时存储区
  private:
	KA_Buzzer buzzer;
  	unsigned char Init_SectorKey[MAX_LEN] = 
	{
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x07, 0x80, 0x69, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
	};
	unsigned char KA_SectorKey[MAX_LEN] = 
	{
		0x31, 0x36, 0x30, 0x38, 0x31, 0x32, 0xFF, 0x07, 0x80, 0x69, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
	};
	int _chipSelectPin;
	int _NRSTPD;
};
extern unsigned char rfidbuf[65];
#endif